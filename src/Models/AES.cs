...
public class AES128 {
    ...
    // untuk konversi subtitution box
    ...
    // inversnya untuk proses dekripsi
    ...
    // round coefficients
    private readonly byte[] roundCoefficient = new byte[ROUND_NUM] {
    0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36 }; //Round coefficient = 10

    // keys
    private byte[] key = {  };

    private byte[][] subkeys = new byte[ROUND_NUM + 1][]; //each subkey is 16-bytes length
    public byte[] Key {
        get { return key; }
        set { 
    ...
    // transformasi keywword dengan subtitution box
    private void keywordTransform (ref byte[] keyword, int roundno) {
        if (roundno < 0 || roundno > ROUND_NUM - 1)
        return;
        byte buf = keyword[0];
    ...
    private void decryptBlock(byte[] ciphertext, ref byte[] plaintext) {
        ...
        for (int r = 0; r < ROUND_NUM; ++r) {
            for (int j = 0; j < BLOCK_SIZE; ++j){
                plaintext[j] ^= Subkeys[ROUND_NUM - r][j];
    ...
    public bool Encrypt(byte[] plaintext, ref byte[] ciphertext) {
        ...
        int extrabytes = plaintext.Length % BLOCK_SIZE; int pblock = (plaintext.Length / BLOCK_SIZE) + (extrabytes > 0 ? 1 : 0); int cblock = ciphertext.Length / BLOCK_SIZE;
        // Console.WriteLine(ciphertext.Length);
        // Console.WriteLine(cblock);
        // Console.WriteLine(pblock);
        if (cblock < pblock){
            return false;
        }
